...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float checkGroundDistance = 0.75f;
    ...
    public float crouchColliderHeightMultiplier = .5f;
    ...
    private void Update()
    {
        ...
        if(readInputs && enableJump && Input.GetKeyDown(jumpKey) && (isGrounded || canFly))
        {
            Jump();
        }
        ...
        if (readInputs && (enableCrouch || canFly))
        {
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            {
                isWalking = true;
                if(targetVelocity.z > 0.3f)
                    GameHud.Instance.questManager.completeQuest("questcatmove");
            }
            else
            {
                isWalking = false;
    ...
    private void Jump()
    {
        ...
        if(canFly)
        {
            GameHud.Instance.questManager.completeQuest("questcatflight");
            Vector3 flyPower = playerCamera.transform.forward * flightPower;
            rb.useGravity = false;
            if (isGrounded)
                flyPower.y = jumpPower;
            //rb.AddForce(flyPower.x, flyPower.y, flyPower.z, ForceMode.Impulse);
            rb.AddForce(0f, flightPower, 0f, ForceMode.Impulse);
            isGrounded = false;

        }
        else if (isGrounded)
        {
            GameHud.Instance.questManager.completeQuest("questcatjump");
    ...
    private void Crouch()
    {
        ...
        //Debug.Log($"flightPower: { canFly } / { isGrounded }");

        if (canFly && !isGrounded)
        {
            rb.AddForce(0f, -flightPower, 0f, ForceMode.Impulse);
            GameHud.Instance.questManager.completeQuest("questcatlower");
        }
        else if (enableCrouch && isCrouched)
        {
        ...
        Array.ForEach(allColliders, collider =>
        {
            if (isCrouched)
                collider.radius *= crouchColliderHeightMultiplier;
            else if (isCrouched)
                collider.radius /= crouchColliderHeightMultiplier;
        });
    }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.canFly = EditorGUILayout.ToggleLeft(new GUIContent("Enable Fly", "Determines if the player is allowed to jump."), fpc.canFly);
        ...
        fpc.flightPower = EditorGUILayout.Slider(new GUIContent("Flight Power", "Determines how high the player will jump."), fpc.flightPower, .1f, 20f);
        
        fpc.checkGroundDistance = EditorGUILayout.Slider(new GUIContent("Check Ground Distance", "Determines how high the player will jump."), fpc.checkGroundDistance, .01f, 20f);
        ...
        fpc.crouchColliderHeightMultiplier = EditorGUILayout.Slider(new GUIContent("Height Multiplier", "Determines the percent 'Walk Speed' is reduced by. 1 being no reduction, and .5 being half."), fpc.crouchColliderHeightMultiplier, .1f, 1);

        //fpc.allColliders = EditorGUILayout(new GUIContent("Speed Reduction", "Determines the percent 'Walk Speed' is reduced by. 1 being no reduction, and .5 being half."), fpc.speedReduction, .1f, 1);
        GUI.enabled = true;
